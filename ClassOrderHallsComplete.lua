...
NS.UpdateLDB = function()
	...
	NS.ldbTooltip.available = C_Garrison.HasGarrison( Enum.GarrisonType.Type_7_0_Garrison );
end
--
...
NS.ldb = LibStub:GetLibrary( "LibDataBroker-1.1" ):NewDataObject( NS.addon, {
	...
	OnClick = function( self, button )
		if button == "RightButton" and self:GetName() == NS.ldbiButtonName then -- Right-Click LibDBIcon Minimap button
			...
			if C_Garrison.HasGarrison( Enum.GarrisonType.Type_7_0_Garrison ) then
				if not GarrisonLandingPage or not GarrisonLandingPage:IsShown() or GarrisonLandingPage.garrTypeID ~= Enum.GarrisonType.Type_7_0_Garrison then
					ShowGarrisonLandingPage( Enum.GarrisonType.Type_7_0_Garrison );
				elseif GarrisonLandingPage:IsShown() and GarrisonLandingPage.garrTypeID == Enum.GarrisonType.Type_7_0_Garrison then
...
NS.Frame( "COHCEventsFrame", UIParent, {
	...
	OnEvent = function ( self, event, ... )
		...
		elseif	event == "GARRISON_FOLLOWER_CATEGORIES_UPDATED"	then
			...
			-- Troops {UPDATED}
			--------------------------------------------------------------------------------------------------------------------------------
			local troops = C_Garrison.GetClassSpecCategoryInfo( Enum.GarrisonFollowerType.FollowerType_7_0_GarrisonFollower );
		...
		elseif	event == "ADDON_LOADED"							then
			...
			-- ADDON_LOADED
			--------------------------------------------------------------------------------------------------------------------------------
			if C_AddOns.IsAddOnLoaded( NS.addon ) and not NS.db then
				self:UnregisterEvent( event );
